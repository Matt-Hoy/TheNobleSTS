package thenoble.cards;

import static thenoble.util.GeneralUtils.removePrefix;
import static thenoble.util.TextureLoader.getCardTextureString;
...
public abstract class NobleCard extends CustomCard {
  ...
  public NobleCard(String ID, CardStats info) {
    this(ID, info, getCardTextureString(removePrefix(ID), info.cardType));
  }

  public NobleCard(String ID, CardStats info, String cardImage) {
    this(
        ID,
        info.baseCost,
        info.cardType,
  ...
  protected final void setCustomVar(
      ...
      VariableType type,
      int base,
      TriFunction<NobleCard, AbstractMonster, Integer, Integer> preCalc) {
  ...
  protected final void setCustomVar(
      ...
      int base,
      TriFunction<NobleCard, AbstractMonster, Integer, Integer> preCalc,
      TriFunction<NobleCard, AbstractMonster, Integer, Integer> postCalc) {
  ...
  protected void setVarCalculation(
      String key, TriFunction<NobleCard, AbstractMonster, Integer, Integer> calculation) {
  ...
  public AbstractCard makeStatEquivalentCopy() {
    ...
    if (card instanceof NobleCard) {
      ...
      ((NobleCard) card).upgradesDescription = this.upgradesDescription;

      ((NobleCard) card).baseCost = this.baseCost;

      ((NobleCard) card).upgradeCost = this.upgradeCost;
      ((NobleCard) card).upgradeDamage = this.upgradeDamage;
      ((NobleCard) card).upgradeBlock = this.upgradeBlock;
      ((NobleCard) card).upgradeMagic = this.upgradeMagic;

      ((NobleCard) card).costUpgrade = this.costUpgrade;
      ((NobleCard) card).damageUpgrade = this.damageUpgrade;
      ((NobleCard) card).blockUpgrade = this.blockUpgrade;
      ((NobleCard) card).magicUpgrade = this.magicUpgrade;

      ((NobleCard) card).baseExhaust = this.baseExhaust;
      ((NobleCard) card).upgExhaust = this.upgExhaust;
      ((NobleCard) card).baseEthereal = this.baseEthereal;
      ((NobleCard) card).upgEthereal = this.upgEthereal;
      ((NobleCard) card).baseInnate = this.baseInnate;
      ((NobleCard) card).upgInnate = this.upgInnate;
      ((NobleCard) card).baseRetain = this.baseRetain;
      ((NobleCard) card).upgRetain = this.upgRetain;

      for (Map.Entry<String, LocalVarInfo> varEntry : cardVariables.entrySet()) {
        LocalVarInfo target = ((NobleCard) card).getCustomVar(varEntry.getKey()),
            current = varEntry.getValue();
        if (target == null) {
          ((NobleCard) card).setCustomVar(varEntry.getKey(), current.base, current.upgrade);
          target = ((NobleCard) card).getCustomVar(varEntry.getKey());
        }
  ...
  private static class QuickDynamicVariable extends DynamicVariable {
    ...
    private NobleCard current = null;
    ...
    public void setIsModified(AbstractCard c, boolean v) {
      if (c instanceof NobleCard) {
        LocalVarInfo var = ((NobleCard) c).getCustomVar(localKey);
    ...
    public boolean isModified(AbstractCard c) {
      return c instanceof NobleCard && (current = (NobleCard) c).isCustomVarModified(localKey);
    }

    @Override
    public int value(AbstractCard c) {
      return c instanceof NobleCard ? ((NobleCard) c).customVar(localKey) : 0;
    }

    @Override
    public int baseValue(AbstractCard c) {
      return c instanceof NobleCard ? ((NobleCard) c).customVarBase(localKey) : 0;
    }

    @Override
    public boolean upgraded(AbstractCard c) {
      return c instanceof NobleCard && ((NobleCard) c).customVarUpgraded(localKey);
    }
  ...
  protected static class LocalVarInfo {
    ...
    TriFunction<NobleCard, AbstractMonster, Integer, Integer> calculation = LocalVarInfo::noCalc;
    ...
    private static int noCalc(NobleCard c, AbstractMonster m, int base) {
      return base;
    }
